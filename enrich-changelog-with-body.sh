...
while IFS= read -r line; do
  ...
  # Scrivi sempre la riga corrente
  echo "$line" >> "$TMPFILE"
    ...
    if [[ "$next_line" == "<!-- COMMIT_DESC -->" ]]; then
      ...
      while IFS= read -r desc_line; do
        if [[ "$desc_line" =~ ^[[:space:]] ]]; then
          ...
          # Se troviamo una riga non indentata, la salviamo per il prossimo ciclo
          line="$desc_line"
          break
        fi
      done
      ...
      # Se avevamo letto una riga, la processiamo normalmente nel prossimo ciclo
      if [[ -n "$next_line" ]]; then
        line="$next_line"
        continue
      fi
