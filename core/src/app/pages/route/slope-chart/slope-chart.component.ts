...
export class SlopeChartComponent implements OnInit {
  ...
  constructor(private _mapService: MapService, private _statusService: StatusService) {
  ...
  private _setChart() {
    if (!!this._chartCanvas && !!this.route) {
      ...
      labels.push(0);
      currentLocation = {
        longitude: this.route.geometry.coordinates[0][0],
        latitude: this.route.geometry.coordinates[0][1],
        altitude: this.route.geometry.coordinates[0][2],
      };
      ...
      for (let i = 1; i < this.route.geometry.coordinates.length; i++) {
        ...
        currentLocation = {
          longitude: this.route.geometry.coordinates[i][0],
          latitude: this.route.geometry.coordinates[i][1],
          altitude: this.route.geometry.coordinates[i][2],
        };
      ...
      for (let i = 1; i < this.route.geometry.coordinates.length && step <= steps; i++) {
