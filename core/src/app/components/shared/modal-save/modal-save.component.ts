...
import {generateUUID, removeCurrentUgcTrackProperties} from '@wm-core/utils/localForage';
...
export class ModalSaveComponent extends BaseSaveComponent implements OnInit {
  ...
  backToMap(): void {
    removeCurrentUgcTrackProperties();
    ...
    from(this._deviceSvc.getInfo())
      .pipe(
        ...
        switchMap(_ => {
          ...
          if (!this.isWaypoint) {
            return from(removeCurrentUgcTrackProperties()).pipe(switchMap(() => of(null)));
          }
          return of(null);
        }),
