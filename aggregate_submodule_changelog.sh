...
if [ -n "$changelog_update" ]; then
  # Usa awk per aggiungere changelog_update all'interno della sezione dell'ultimo tag
  awk -v changelog_update="$changelog_update" -v last_tag="## [$last_tag]" '
    $0 ~ last_tag { print; print changelog_update; next }
    { print }
