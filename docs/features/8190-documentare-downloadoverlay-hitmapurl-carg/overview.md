...
- [ ] Documentare l'hardcoding `[overlayXYZ]="'https://carg.geosciences-ir.it/storage/cargmap/'"` in `map.page.html:209`, incluso il fatto che sovrascrive il default (`https://api.webmapp.it/tiles`) definito in `download-panel.component.ts`
- [ ] Documentare che download overlay e basemap geologico condividono la stessa origine tile (`carg.geosciences-ir.it/storage/cargmap/`), ma restano meccanismi distinti: il tile layer OL in `map-core/src/directives/hit-map.directive.ts:134` (visualizzazione sulla mappa) vs `overlayXYZ` propagato a `downloadOverlay()` in `localForage.ts:282` (download offline)
