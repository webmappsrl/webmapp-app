...
- **Task 1 (bump submodule wm-core) non eseguito in questa sessione**: nessun commit esiste ancora su wm-core da puntare (i commit avvengono solo dopo approvazione del developer al review-gate). Le modifiche vivono nel working tree del submodule sullo stesso branch feature e sono già risolvibili via alias `@wm-core/*` per la compilazione — ma il bump pin reale (commit dedicato "Updated submodule core/src/app/shared/wm-core") resta da fare come primo step dopo i commit su wm-core.
