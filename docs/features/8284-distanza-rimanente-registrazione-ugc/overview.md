...
- `resume()` ora dispatcha anch'esso `setMapDetailsStatus({status:'onlyTitle'})` — senza questo fix, riprendere la registrazione dopo una pausa in cui l'utente avesse riaperto un pannello di dettaglio (che lo forza sempre a `'open'`) lasciava il pannello a piena altezza durante il movimento, contraddicendo lo scopo del dispatch già presente in `recordStart()`
- La logica del view-model (Requisiti sotto) è stata estratta in un selettore condiviso `trackLiveDistanceVm` in wm-core, invece di restare duplicata identica tra questo componente e `tab-detail.component.ts` — vedi overview lato wm-core
- Rimossa la chiave i18n orfana `pages.register.time` (non più referenziata dopo la rimozione dell'etichetta "IN MOVIMENTO") da tutti i 7 file di lingua del repo principale
- `.wm-recorder-infopanel-top-center` (il timer) non dipende più dall'emissione del view-model dei badge — prima un fallimento/assenza di emissione dei 4 selettori avrebbe fatto scomparire anche il cronometro
...
- [ ] `track-recorder.component.ts`: `trackLiveDistanceVm$ = this._store.select(trackLiveDistanceVm)` — riusa il selettore condiviso `trackLiveDistanceVm` (wm-core, `user-activity.selector.ts`, estratto durante la review formale da una logica inizialmente duplicata per `combineLatest` su `trackDistanceCovered`/`trackRemainingDistance`/`trackPositionStale`/`confOPTIONSShowTrackRemainingDistance`) — **stesso gate già usato da `tab-detail.component.ts` (oc:8177)**, ora nello stesso identico selettore: se `confOPTIONSShowTrackRemainingDistance` è `false`, i due valori restano `null` a prescindere dal dato reale
- [ ] `track-recorder.component.html`: `.wm-recorder-infopanel-top` ristrutturato con due nuove colonne laterali (`.wm-recorder-infopanel-top-side`), prima e dopo `.wm-recorder-infopanel-top-center` (che resta invariata e **non** è più condizionata dall'emissione del view-model, a differenza di una prima versione — vedi Rischi). Ciascuna colonna: label uppercase xsm ("PARTENZA"/"ARRIVO", chiavi i18n `from`/`to`) + `<wm-track-live-distance-badge [distanceMeters]="..." [stale]="...">`, con `*ngIf` sul valore per non renderizzare l'etichetta quando il badge è assente (il badge gestisce già internamente `*ngIf="distanceMeters != null"` sul proprio contenuto, ma la label va nascosta esplicitamente altrimenti resterebbe orfana)
...
- **Colonne laterali condizionate dall'emissione di `trackLiveDistanceVm$`** — ciascuna colonna è avvolta in un `*ngIf="trackLiveDistanceVm$|async as liveDistance"` indipendente (due sottoscrizioni distinte allo stesso selettore, per non condizionare anche il timer centrale, vedi fix post-review); in pratica il selettore NgRx emette sempre in modo sincrono dallo stato iniziale del reducer, quindi le due colonne sono a tutti gli effetti sempre presenti quando il componente vive
...
- `src/app/components/track-recorder-component/track-recorder.component.ts` — incluso il dispatch di `setMapDetailsStatus({status:'onlyTitle'})` in `recordStart()` **e in `resume()`** (fix post-review), riuso del selettore condiviso `trackLiveDistanceVm`
...
- `src/assets/i18n/{it,en,de,es,fr,pr,sq}.ts` — rimossa la chiave orfana `pages.register.time` (fix post-review, nessuna nuova chiave)

**wm-core (submodule):** vedi `core/src/app/shared/wm-core/docs/features/8284-distanza-rimanente-registrazione-ugc/overview.md` — `REMAINING_DISTANCE_MAX_SPEED_MS`, `@Input() showSuffix` su `WmTrackLiveDistanceBadgeComponent`, e (fix post-review) nuovo selettore condiviso `trackLiveDistanceVm` in `user-activity.selector.ts` + aggiornamento di `tab-detail.component.ts` per riusarlo.
