...
function updateAndroidPlatform(instanceName, appId, appName) {
  return new Promise((resolve, reject) => {
    ...
    promises.push(
      new Promise((resolve, reject) => {
        gulp
          ...
          .pipe(replace(/compileSdkVersion = ([0-9]{2})/g, 'compileSdkVersion = 30'))
          .pipe(replace(/targetSdkVersion = ([0-9]{2})/g, 'targetSdkVersion = 30'))
    ...
    promises.push(
      new Promise((resolve, reject) => {
        gulp
          .src(instancesDir + instanceName + '/android/app/src/main/res/values/strings.xml')
          .pipe(
            replace(
          ...
          .pipe(replace(/<string name="plugin_bgloc_account_name">Webmapp<\/string>/g, ''))
