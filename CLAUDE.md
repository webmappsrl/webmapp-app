...
### Debito tecnico risolto: `--configuration=production` ora abilitato su entrambi i deploy web

oc:8382 ha risolto gli errori di compilazione che bloccavano `--configuration=production` (dead code `PoiPage`, tipo `IHIT` disallineato, collisione dipendenze `domhandler`). Dopo il merge di quel fix su questo branch, è emerso un ulteriore errore specifico di RDO in `favourites.page.html` (`ion-segment`/`SegmentValue` non assegnabile a `'tracks' | 'layers'`, risolto con `$any()` sul valore dell'evento — i due `ion-segment-button` del template usano solo quei due valori, il cast è sicuro). Verificato con build reale che sia `ionic build --configuration=production` sia la combinazione `--configuration=production,camminiditalia` (mai testata prima) completano senza errori. `deploy-to-web-default.js` e `deploy-to-web-camminiditalia.js` ora usano entrambi la build di produzione.
...
| Abilitare build di produzione sui deploy web dedicati (RDO camminiditalia) | oc:8382 | `core/scripts/deploy-to-web-default.js`, `core/scripts/deploy-to-web-camminiditalia.js`, `core/src/app/pages/favourites/favourites.page.html` | Follow-up dopo il merge di oc:8382 su questo branch: entrambi gli script deploy-to-web-* ora usano `--configuration=production` (rispettivamente da solo e combinato con `camminiditalia`); fix di un ulteriore errore AOT specifico di RDO (`ion-segment`/`SegmentValue` in favourites.page.html) mai emerso prima perché nessuna build production era mai stata tentata su questo branch |
| "Cerca il tuo cammino" — filtri sui cammini in Home (camminiditalia) | oc:8414 | `core/angular.json` (`fileReplacements` su `search-bar.component.ts`), `core/cypress/e2e/app_52/route-filters.cy.ts`, `core/cypress/fixtures/conf-route-filters.json`; componenti/store in wm-core, tipi in wm-types | Pannello filtri (Lunghezza/Tappe/Tipologia/Portata/Regioni/Temi/Stagioni) fedele al riferimento reale camminiditalia.org, instradato via `fileReplacements` sulla `wm-searchbar` preesistente (non un componente nuovo montato accanto). Filtri attivi (anche senza testo digitato) mostrano la vista risultati a tab forzata sulla sola tab Layers. Dettagli completi in `core/src/app/shared/wm-core/docs/features/8414-filtri-cammini-home/notes.md` |
| Accordion wm-config-detail: apertura multipla e rimozione scrollIntoView | oc:8458 | `core/src/app/pages/map/map-details/map-details.component.ts`, `core/src/app/pages/map/map.page.html`, `core/cypress/e2e/app_52/config-detail-boxes.cy.ts`, `core/cypress/utils/test-utils.ts`; componente in wm-core, tipo rimosso in wm-types | Rimosso lo scroll automatico (`onConfigDetailSettled()`/`_isFullyInView()`) dal pannello mappa, perché `wm-config-detail` (wm-core) smette di dispacciare `configDetailSettled`. Comportamento condiviso da tutti gli shard, nessuna eccezione. Dettagli completi in `docs/features/8458-accordion-wm-config-detail-apertura-multipla/notes.md` |
| Preferiti: default tab Cammini invece di Sentieri | oc:8465 | `core/src/app/pages/favourites/favourites.page.ts`, `core/src/app/pages/favourites/favourites.page.spec.ts` | Il default segue `showLayersSegment$` in modo reattivo (non un gate one-shot su `isConfLoaded`), con guard su scelta manuale utente e reset a `'tracks'` se il flag torna `false`. Comportamento condiviso da tutti gli shard, nessuna eccezione. Dettagli completi in `docs/features/8465-preferiti-aprire-di-default-il-tab-cammini-non-sentieri/notes.md` |

## Decisioni architetturali

### Preferiti: default tab Cammini invece di Sentieri (oc:8465)
- **Un gate one-shot su `isConfLoaded` (`take(1)`) è insufficiente** contro la doppia emissione di `getConf()` (`handleApiCache`: prima la cache, poi la fetch fresca) — `isConfLoaded` è un latch che diventa `true` alla prima `loadConfSuccess`, spesso quella dalla cache stale, congelando il default sul valore vecchio. Scoperto in review formale (`wm-skills:wm-review-ticket`), non in Fase: challenge né dai test della prima implementazione (tutti scritti con `of()`, che non può esercitare transizioni multiple nel tempo). Il design finale segue direttamente `showLayersSegment$` (già `false` finché la config reale non arriva, per costruzione del reducer wm-core) con una sottoscrizione continua, non un `take(1)`.
- **Guard `_userSelectedSegmentManually`** (settato in `onSegmentChange()`) disabilita il sync automatico una volta che l'utente ha scelto un tab — senza, una scelta manuale fatta durante un cold start lento veniva sovrascritta appena la config arrivava.
- **Reset a `'tracks'` se il flag torna `false`** dopo essere stato `true`: evita un vicolo cieco (nessuno switch per tornare a Sentieri) se la config si ricarica con un flag diverso a sessione già avviata — bug preesistente nel componente, reso raggiungibile con zero interazione dal nuovo default.
- **`OnDestroy`/`Subject`/`takeUntil` aggiunti per il teardown esplicito** della subscription (pattern già in uso nel repo, `track-recorder.component.ts`, `modal-success.component.ts`) — assente nella prima versione, dove un `take(1)` che non si risolve mai (config mai caricata, offline) avrebbe lasciato la subscription viva per tutta la sessione.
- **Rischio residuo accettato, non mitigato**: se l'utente sta usando la tab tracce (senza mai toccare lo switch) e il flag passa da `false` a `true` a sessione già avviata (es. riconnessione → nuovo `loadConf()`), il sync continuo lo sposterebbe comunque su "Cammini" perdendo la posizione di scroll — il guard si attiva solo su un tap esplicito, non sul semplice uso del contenuto. Probabilità giudicata molto bassa, impatto contenuto (nessuna perdita dati); non affrontato per non introdurre un secondo concetto di "interazione utente". Dettagli completi in `docs/features/8465-preferiti-aprire-di-default-il-tab-cammini-non-sentieri/notes.md`.
- **Rinomina label "Layers"→"Cammini" esplicitamente fuori scope**: gestita lato backend via override `config.json → TRANSLATIONS` per lo shard camminiditalia sulla chiave i18n esistente (stesso meccanismo già usato in oc:7643 per la chiave `layers` minuscola) — nessuna modifica frontend.
- **Verifica e2e (`favourites.cy.ts`) eseguita ma non verde**: fallisce identicamente con e senza il fix (verificato via `git stash` per isolamento) — causa la mismatch tra shard servito in locale e `appId`/conf attesi dagli spec `app_52`, non il codice applicativo. Non corretto in questo ciclo (ambiente di test locale, fuori scope).

### Accordion wm-config-detail: apertura multipla e rimozione scrollIntoView (oc:8458)
- **Comportamento condiviso da tutti gli shard**, nessun `fileReplacements`, nessuna eccezione per camminiditalia — deciso esplicitamente in reverse-interaction nonostante il ticket sia nato sul branch RDO camminiditalia: `wm-config-detail` non ha mai avuto personalizzazioni per-shard, e non ne introduce con questo ticket.
- **Rimozione integrale (non isolamento/genericizzazione) del meccanismo di assestamento** introdotto in oc:8427 (`transitionend`/debounce/fallback/`CustomEvent('configDetailSettled')`) — il suo unico scopo (scroll automatico) è stato eliminato nello stesso ciclo, quindi l'intera infrastruttura di notifica è stata rimossa insieme, non solo disattivata. Rischio accettato: se in futuro servisse un hook "item aperto e visibile" (es. analytics), andrebbe riscritto da zero.
- **Nessun flag `OPTIONS.*` di rollout**, a differenza del pattern consolidato in questo progetto per altre modifiche UX condivise (`showTrackRemainingDistance` oc:8177, `ugcTrackShareEnabled` oc:8183, `showFavorites` oc:8176) — deciso in Fase: challenge, comportamento di interazione minore non giudicato abbastanza rischioso da giustificare un kill switch dedicato.
- **Apertura multipla illimitata**, nessun tetto al numero di item aperti simultaneamente e nessun meccanismo "chiudi tutto" — rischio di performance con contenuto pesante (iframe multipli) accettato esplicitamente, il contenuto di `config_detail` è curato dal backend/content editor, non generato dall'utente finale.
- **Bug preesistente trovato e risolto in `cypress/utils/test-utils.ts` → `openPoi()`**: mancava `.scrollIntoView()` prima di `.should('be.visible')`, causando fallimenti intermittenti (`position: fixed` overflow) su qualunque spec che apra un POI dai risultati Home — non correlato a `wm-config-detail`, scoperto eseguendo realmente il nuovo test Cypress di questo ticket. Fix minimo, nessun altro chiamante impattato negativamente.

### "Cerca il tuo cammino" — filtri sui cammini in Home (oc:8414)
- **Un solo `fileReplacements` in `angular.json`**, su `search-bar.component.ts` → `search-bar.component.camminiditalia.ts`: il piano originale ne prevedeva due, su un componente `home-route-filters` nuovo di zecca mai esistito prima di questo ticket — architettura ripensata in corso d'opera dopo che il developer ha fatto notare che, a differenza del precedente `wm-home-layer` (oc:8391, componente preesistente comune a tutti gli shard), non aveva senso mantenere una "versione generica" per una feature interamente nuova. Il pannello filtri è quindi diventato parte della variante camminiditalia della searchbar preesistente stessa.
- **`SearchBarBaseComponent` (wm-core) estratta come base condivisa** tra la searchbar di default e la variante camminiditalia — deviazione motivata rispetto alla regola generale di questo file ("non estrarre una classe base", vedi sopra "Personalizzazioni per-shard via fileReplacements"): qui non è l'intero componente duplicato con solo lo stile diverso, è un sottoinsieme di logica di ricerca genuinamente condiviso al 100%, con la variante che aggiunge funzionalità invece di duplicarla. Dettagli/motivazione completa in `wm-core/CLAUDE.md`.
- **Verificato con build reale** `ng build --configuration=camminiditalia` (oltre ai test unitari su `home-route-filters.utils.ts` e alla verifica manuale via dev server).
- **Debito noto**: test Cypress (`route-filters.cy.ts`) scritto e rivisto staticamente ma non eseguito realmente in CI/locale in questo ciclo; icone Portata/Regioni non aggiornate alle esatte SVG del sito (il developer ha fornito solo Lunghezza/Tipologia/Temi/Stagioni); animazione apertura/chiusura limitata al pannello, non estesa alle singole righe filtro.
